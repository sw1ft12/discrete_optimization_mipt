...
def aco(n, m, sets, costs, time_limit=90, ants=25, rho=0.98):
    ...
    while time.time() - start < time_limit:
        ...
        if no_improve >= 50:
            for i in range(m):
                pheromone[i] = random.uniform(tau_min, tau_max)
            no_improve = 0
            continue
...
if __name__ == "__main__":
    ...
    for i in range(3):
        sol, cost = aco(n, m, sets, costs, time_limit=90)
        if cost < best_cost:
            best_sol = sol
            best_cost = cost
